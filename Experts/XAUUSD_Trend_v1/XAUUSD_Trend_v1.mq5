...
//|  Instruments: XAUUSD                                            |
//|  Version: 1.2 - H4 ADX trend strength filter                    |
//+------------------------------------------------------------------+
...
input int    H4_EMA_Fast     = 20;       // H4 Fast EMA period
input int    H4_EMA_Slow     = 50;       // H4 Slow EMA period
...
input double ATR_Trail_Multi = 1.5;      // ATR multiplier for trailing stop
input int    ATR_Period      = 14;       // ATR period
input int    MaxUnprotected  = 2;        // Max positions not yet at breakeven

input group "=== Trade Settings ==="
input int    MagicNumber     = 20240001; // Magic number
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      // Position cooldown: only treat as "protected" when SL is exactly at breakeven.
      // Once trailing stop moves SL above open_price, position re-occupies an unprotected slot,
      // preventing new entries at extended prices. This naturally filters late-trend chasing.
      bool is_trailing = (MathAbs(current_sl - open_price) < _Point * 5);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      // Same cooldown logic: SL above open_price means position is trending but slot stays occupied
